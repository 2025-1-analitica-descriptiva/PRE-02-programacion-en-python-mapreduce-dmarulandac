...
def load_input(input_directory):
    ...
    with fileinput.input(files=files) as f:
        for line in f:
            sequence.append((fileinput.filename(), line)) # una tupla con el nombre del archivo seguido de una linea
            #lo devuelve como una secuencia
    return sequence
...
def shuffle_and_sort(sequence):
    """Shuffle and Sort"""
    #Se ordena la tupla directamente, con un método diferente al establecido en py
    return sorted(sequence, key=lambda x: x[0])
    #elementos con la misma clave consecutivos
#
...
def reducer(sequence):
    ...
    for key, group in groupby(sequence, lambda x: x[0]):
        result.append(
            (
                key, #la clave. El grupo es la lista de tuplas
                sum(value for _, value in group)))#La agrupación y conteo de palabras
    return result
#
